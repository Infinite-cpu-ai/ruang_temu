<x-guest-layout>
    <div class="relative rounded-[2.5rem] border border-white/40 bg-white/70 backdrop-blur-xl p-8 sm:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
        ...
        <div class="mb-10 text-center relative z-10">
            <h1 class="text-3xl font-extrabold tracking-tight text-gray-900 mb-3">Selamat Datang</h1>
            <p class="text-sm font-medium text-gray-500 max-w-sm mx-auto">
                Masuk untuk konsultasi dan kelola proyek impianmu.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-6 relative z-10">
            ...
            <div>
                ...
                <input id="password" type="password" name="password" required autocomplete="current-password" 
                    class="block w-full rounded-2xl border-gray-200 bg-white/80 px-4 py-3 placeholder-gray-400 focus:border-gray-900 focus:ring-gray-900 transition shadow-sm text-sm" placeholder="••••••••">
            ...
            <div class="flex items-center justify-between pt-1">
                <label for="remember_me" class="inline-flex items-center gap-2 cursor-pointer group">
                    <input id="remember_me" type="checkbox" class="w-4 h-4 rounded border-gray-300 text-gray-900 focus:ring-gray-900 transition" name="remember">
                    <span class="text-sm font-medium text-gray-500 group-hover:text-gray-900 transition">Ingat saya</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-sm font-bold text-gray-400 hover:text-gray-900 transition" href="{{ route('password.request') }}">
                        Lupa password?
                    </a>
                @endif
            </div>
        ...
        <div class="mt-10 pt-6 border-t border-gray-100 text-center relative z-10">
            <p class="text-sm font-medium text-gray-500">
                Belum punya akun?
                <a href="{{ route('register') }}" class="font-bold text-gray-900 hover:underline decoration-2 underline-offset-4 ml-1">Daftar sekarang</a>
            </p>
        </div>
    </div>
</x-guest-layout>
