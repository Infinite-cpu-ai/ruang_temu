        ...
        <div class="relative z-10 mb-8" x-data="{ role: '{{ old('role', 'user') }}' }">
            <div class="flex rounded-2xl bg-gray-100 p-1 gap-1">
                <button type="button" @click="role = 'user'"
                    :class="role === 'user' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                ...
                <button type="button" @click="role = 'architect'"
                    :class="role === 'architect' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                    class="flex-1 flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold transition-all duration-200">
            ...
            <form method="POST" action="{{ route('register') }}" class="mt-8 space-y-6">
                ...
                <!-- Context note -->
                <div x-show="role === 'architect'" class="rounded-2xl bg-gray-50 border border-gray-100 px-4 py-3 text-xs text-gray-500 font-medium" x-cloak>
                ...
                <div class="pt-2">
                    <button type="submit" class="w-full inline-flex justify-center items-center gap-2 rounded-full border border-transparent bg-gray-900 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-gray-900/20 hover:bg-black focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 transition-all active:scale-[0.98]">
                        <span x-text="role === 'architect' ? 'Daftar sebagai Arsitek' : 'Daftar sebagai Klien'">Daftar sebagai Klien</span>
