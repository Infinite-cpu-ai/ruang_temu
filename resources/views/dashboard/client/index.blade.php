...
<div class="relative min-h-screen bg-[#FAFAFA] overflow-x-hidden">
    ...
    <div class="relative z-10 max-w-5xl mx-auto px-6 py-14 pt-28">
        ...
        <!-- Header -->
        <div class="mb-12">
            <p class="text-sm font-semibold text-gray-400 tracking-widest uppercase mb-2">Dashboard</p>
            <h1 class="text-4xl font-extrabold tracking-tight text-gray-900">Halo, {{ auth()->user()->name }} 👋</h1>
            <p class="mt-2 text-base text-gray-500 font-medium">Pantau proyek dan temukan arsitek terbaik untuk hunianmu.</p>
        </div>

        <!-- Stats Grid -->
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
            <div class="relative rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl p-6 shadow-[0_4px_20px_rgb(0,0,0,0.04)] overflow-hidden">
                <div class="text-3xl font-extrabold text-gray-900">{{ $totalProjects }}</div>
            ...
            <div class="relative rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl p-6 shadow-[0_4px_20px_rgb(0,0,0,0.04)] overflow-hidden">
                <div class="text-3xl font-extrabold text-emerald-600">{{ $completedProjects }}</div>
                <div class="mt-1 text-xs font-semibold text-gray-400 tracking-wide">Selesai</div>
            </div>
            <div class="relative rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl p-6 shadow-[0_4px_20px_rgb(0,0,0,0.04)] overflow-hidden">
                <div class="text-3xl font-extrabold text-gray-900">{{ $followingCount }}</div>
                <div class="mt-1 text-xs font-semibold text-gray-400 tracking-wide">Arsitek Diikuti</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl p-7 shadow-[0_4px_20px_rgb(0,0,0,0.04)]">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-lg font-extrabold text-gray-900">Proyek Terbaru</h2>
                    <a href="{{ route('client.projects.index') }}" class="text-sm font-bold text-gray-400 hover:text-gray-900 transition">Lihat semua →</a>
                </div>
                ...
                @forelse($recentProjects as $project)
                    <div class="flex items-center justify-between py-3.5 border-b border-gray-100 last:border-0">
                        <div>
                            ...
                            <div class="text-xs text-gray-500 mt-0.5">Arsitek: {{ $project->architect?->name ?? '-' }}</div>
                        </div>
                        @php
                ...
                @empty
                    <div class="py-8 text-center text-sm text-gray-400 font-medium">
                        Belum ada proyek. Mulai cari arsitek!
                    </div>
                @endforelse
            </div>
            ...
            <div class="flex flex-col gap-4">
                <div class="rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl p-7 shadow-[0_4px_20px_rgb(0,0,0,0.04)]">
                    ...
                    <div class="flex flex-col gap-3">
                        <a href="{{ route('features.cari') }}" class="flex items-center gap-3 rounded-2xl bg-gray-900 text-white px-4 py-3 text-sm font-bold hover:bg-black transition active:scale-[0.97]">
                            <span class="flex items-center justify-center w-7 h-7 rounded-full bg-white/10">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                            </span>
                            Cari Arsitek
                        </a>
                        <a href="{{ route('client.projects.index') }}" class="flex items-center gap-3 rounded-2xl bg-white border border-gray-200 text-gray-900 px-4 py-3 text-sm font-bold hover:bg-gray-50 transition active:scale-[0.97]">
                        ...
                        <a href="{{ route('client.reviews.index') }}" class="flex items-center gap-3 rounded-2xl bg-white border border-gray-200 text-gray-900 px-4 py-3 text-sm font-bold hover:bg-gray-50 transition active:scale-[0.97]">
                            <span class="flex items-center justify-center w-7 h-7 rounded-full bg-gray-100">
                        ...
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 rounded-2xl bg-white border border-gray-200 text-gray-900 px-4 py-3 text-sm font-bold hover:bg-gray-50 transition active:scale-[0.97]">
                            <span class="flex items-center justify-center w-7 h-7 rounded-full bg-gray-100">
